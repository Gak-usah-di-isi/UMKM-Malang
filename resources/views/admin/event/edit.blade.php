...
@section('content')
    <main class="flex-1 p-6">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="mb-8">
                <nav class="mb-6">
                    <div class="flex items-center space-x-2 text-sm text-gray-600">
                        ...
                        <a href="{{ route('admin.events.show', $event->slug) }}"
                            class="hover:text-blue-600">{{ Str::limit($event->title, 30) }}</a>
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                        </svg>
                ...
                <p class="text-gray-600">Perbarui informasi event</p>
            </div>
            ...
            <form action="{{ route('admin.events.update', $event->slug) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="bg-white shadow-lg rounded-xl p-6 border border-gray-100">
                    ...
                    @if ($event->thumbnail)
                        <div id="current-thumbnail" class="mb-4">
                            ...
                            <div class="relative group aspect-video max-w-md">
                                ...
                                <button type="button" id="change-thumbnail"
                                    class="absolute inset-0 bg-black bg-opacity-50 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    ...
                    <label for="thumbnail-input" class="block cursor-pointer {{ $event->thumbnail ? 'hidden' : '' }}">
                        <div
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="content" rows="12" id="content-editor"
                                ...
                                placeholder="Tulis deskripsi event di sini...">{{ old('content', $event->content) }}</textarea>
                            @error('content')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <script>
        ...
        // Show upload area when change thumbnail is clicked
        if (changeThumbnail) {
