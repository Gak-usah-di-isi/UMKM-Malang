...
@section('content')
    <main class="flex-1 p-6">
        <div class="max-w-6xl mx-auto">
            <div class="mb-6">
                <h1 class="text-3xl font-bold text-gray-800">Edit Kategori UMKM</h1>
                <p class="text-gray-600">Perbarui kategori UMKM: {{ $category->name }}</p>
            </div>

            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <form action="{{ route('admin.umkm-categories.update', $category->slug) }}" method="POST">
                    @csrf
                    @method('PUT')
